...
// NOTE: fixed to interpolate Z linearly across the arc so arcs are 3D/helical when start.z != end.z
function interpolateArcXYPoints(start, end, center, cw) {
    const sx = start.x, sy = start.y, sz = (start.z != null ? start.z : 0);
    const ex = end.x, ey = end.y, ez = (end.z != null ? end.z : sz);
    ...
    for (let i = 1; i <= segments; i++) {
        ...
        const z = sz + (ez - sz) * t; // <<< linear Z interpolation across arc
        points.push({ x, y, z });
    }
...
function handleArc(gCode, params) {
    ...
    // push interpolated points for visualization (these now include interpolated Z)
    ...
    const finalDx = target.x - last.x, finalDy = target.y - last.y, finalDz = (target.z != null ? target.z : start.z) - (last.z != null ? last.z : start.z);
